...
// Note: We're no longer importing Pose from '@mediapipe/pose'
import { Results, POSE_LANDMARKS } from '@mediapipe/pose';
...
const LIMB_VECTORS = [
  ['LEFT_WRIST', 'LEFT_ELBOW'], ['LEFT_ELBOW', 'LEFT_SHOULDER'], ['LEFT_SHOULDER', 'RIGHT_SHOULDER'],
  ['RIGHT_WRIST', 'RIGHT_ELBOW'], ['RIGHT_ELBOW', 'RIGHT_SHOULDER'], ['LEFT_SHOULDER', 'LEFT_HIP'],
  ['RIGHT_SHOULDER', 'RIGHT_HIP'], ['LEFT_HIP', 'RIGHT_HIP'], ['LEFT_HIP', 'LEFT_KNEE'],
  ['LEFT_KNEE', 'LEFT_ANKLE'], ['RIGHT_HIP', 'RIGHT_KNEE'], ['RIGHT_KNEE', 'RIGHT_ANKLE']
] as const;

const FRAME_INTERVAL = 30; // Process every 30th frame
let frameCount = 0;

export default function WorkoutTracker({ exerciseName, difficulty }: WorkoutTrackerProps) {
  ...
  useEffect(() => {
    ...
    let referencePose: any;

    async function initReferencePose() {
      try {
        ...
        await new Promise((res, rej) => {
          const v = referenceVideoRef.current!;
          v.onloadeddata = res;
          v.onerror = rej;
          v.load();
        });
        ...
        referencePose = new Pose({ locateFile: file => `https://cdn.jsdelivr.net/npm/@mediapipe/pose@0.5.1675469404/${file}` });
        await referencePose.initialize();
        referencePose.setOptions({ modelComplexity: 1, smoothLandmarks: true, enableSegmentation: true, smoothSegmentation: true, minDetectionConfidence: 0.5, minTrackingConfidence: 0.5 });
        ...
        referencePose.onResults(results => {
          ...
          if (results.poseLandmarks && isRefTracking) {
            if (frameCount % FRAME_INTERVAL === 0) setReferencePoseSequence(prev => [...prev, results.poseLandmarks]);
            frameCount++;
          }
          // drawConnectors / drawLandmarks calls...
          ctx.restore();
        });

        if (isRefTracking) {
          await referenceVideoRef.current!.play();
          const process = async () => { if (!referenceVideoRef.current!.paused) { await referencePose.send({ image: referenceVideoRef.current! }); requestAnimationFrame(process); } };
          process();
        } else referenceVideoRef.current.pause();
      } catch (error) {
        toast({ title: 'Error', description: 'Failed to initialize reference pose detection.', variant: 'destructive' });
      }
    }

    initReferencePose();
    return () => { referencePose?.close(); };
  }, [isRefTracking, toast]);

  // Initialize user video & pose detection
  useEffect(() => {
    if (!videoRef.current || !canvasRef.current || !isUserTracking) return;
    let userPose: any;

    async function initUserPose() {
      try {
        await loadPoseScript();
        const Pose = (window as any).Pose;

        userPose = new Pose({ locateFile: file => `https://cdn.jsdelivr.net/npm/@mediapipe/pose@0.5.1675469404/${file}` });
        await userPose.initialize();
        userPose.setOptions({ modelComplexity: 1, smoothLandmarks: true, enableSegmentation: true, smoothSegmentation: true, minDetectionConfidence: 0.5, minTrackingConfidence: 0.5 });
        ...
        userPose.onResults(results => {
          ...
          ctx.save(); ctx.clearRect(0, 0, ctx.canvas.width, ctx.canvas.height);
          ctx.drawImage(results.image, 0, 0, ctx.canvas.width, ctx.canvas.height);
          if (results.poseLandmarks) setUserPoseSequence(prev => [...prev, results.poseLandmarks]);
          ctx.restore();
        });

        const camera = new Camera(videoRef.current, { onFrame: async () => { await userPose.send({ image: videoRef.current! }); }, width: 640, height: 480 });
        camera.start();
      } catch (error) {
        toast({ title: 'Error', description: 'Failed to initialize user pose detection.', variant: 'destructive' });
      }
    }

    initUserPose();
    return () => { /* camera stop, userPose.close() if needed */ };
  }, [isUserTracking, toast]);

  const startTracking = () => { setIsUserTracking(true); setIsRefTracking(true); setScore(null); setBestScore(null); setUserPoseSequence([]); setReferencePoseSequence([]); toast({ title: 'Tracking Started' }); };
  const stopTracking = () => { setIsUserTracking(false); setIsRefTracking(false); if (videoRef.current?.srcObject) (videoRef.current.srcObject as MediaStream).getTracks().forEach(t => t.stop()); toast({ title: 'Tracking Stopped' }); };

  return (
    <Card className="w-full max-w-4xl mx-auto">
      <CardHeader>
        <div className="flex items-center gap-2">
          <Button variant="ghost" size="icon" onClick={() => navigate('/home')}><ArrowLeft size={20} /></Button>
          <CardTitle>{exerciseName} - {difficulty}</CardTitle>
        </div>
      </CardHeader>
      <CardContent>
        {/* Video & canvas layout omitted for brevity */}
        <div className="flex justify-center space-x-4">
          {!isUserTracking ? <Button onClick={startTracking}>Start Workout</Button> : <Button variant="destructive" onClick={stopTracking}>Stop Workout</Button>}
        </div>
      </CardContent>
    </Card>
  );
